...
class SparkDataCheck:
    ...
    #------------------------------------------------------
    @classmethod    
    def from_csv(cls, spark, path, sep = ";"):
        df = (spark.read
                   .format("csv")
                   .option("header", True)
    ...
    # ----------------------------------------------------------------------------------
    # 2.1 define a method to report min and max of a numeric column supplied by the user
    # ----------------------------------------------------------------------------------
    ...
    def min_max(self, col = None, group = None):
        ...
        if group is not None:
            ...
            # merge all grouped results into one DataFrame
            merged = reduce(lambda left, right: pd.merge(left, right, on = group),dfs)
            return merged

        #----------------------------------------------
        # ungrouped scenario for all numeric columns
        agg_exprs = []
        for c in numeric_cols:
            agg_exprs.extend([
